...
import static technology.rocketjump.saul.rendering.RenderMode.DIFFUSE;
import static technology.rocketjump.saul.rendering.RenderMode.NORMALS;
...
public class ParticleEmitter {
	...
	public void update (float delta) {
		...
		for (int i = 0, n = active.length; i < n; i++) {
			if (active[i]) {
				boolean isAlive = updateParticle(this.diffuseParticles[i], diffuseSprites, delta, deltaMillis, DIFFUSE);
				if (usesNormalsRendering) {
					updateParticle(this.normalParticles[i], normalSprites, delta, deltaMillis, NORMALS);
				}
				if (!isAlive) {
					active[i] = false;
					activeCount--;
	...
	public void draw(Batch batch, RenderMode renderMode) {
		...
		Particle[] particles = renderMode.equals(DIFFUSE) ? this.diffuseParticles : this.normalParticles;
		boolean[] active = this.active;
	...
	private void activateParticle (int index) {
		...
		activateParticle(index, diffuseSprites, diffuseParticles, new RandomXS128(seed), DIFFUSE);
		if (usesNormalsRendering) {
			activateParticle(index, normalSprites, normalParticles, new RandomXS128(seed), NORMALS);
		}
	}
	...
	private boolean updateParticle(Particle particle, Array<Sprite> sprites, float delta, int deltaMillis, RenderMode renderMode) {
		...
		float[] color;
		if ((updateFlags & UPDATE_TINT) != 0 && renderMode.equals(DIFFUSE))
