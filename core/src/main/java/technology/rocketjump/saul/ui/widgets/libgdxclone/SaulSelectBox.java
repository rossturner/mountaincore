...
package technology.rocketjump.saul.ui.widgets.libgdxclone;
 ...
 * @author mzechner
 * @author Nathan Sweet */
public class SaulSelectBox<T> extends Widget implements Disableable {
	static final Vector2 temp = new Vector2();

	SelectBox.SelectBoxStyle style;
	final Array<T> items = new Array();
	final ArraySelection<T> selection = new ArraySelection(items);
	SaulSelectBoxList<T> saulSelectBoxList;
	private float prefWidth, prefHeight;
	private ClickListener clickListener;
	boolean disabled;
	private int alignment = Align.left;

	public SaulSelectBox (Skin skin) {
		this(skin.get(SelectBox.SelectBoxStyle.class));
	}

	public SaulSelectBox (Skin skin, String styleName) {
		this(skin.get(styleName, SelectBox.SelectBoxStyle.class));
	}

	public SaulSelectBox (SelectBox.SelectBoxStyle style) {
		setStyle(style);
		...
		saulSelectBoxList = new SaulSelectBoxList(this);

		addListener(clickListener = new ClickListener() {
			public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
				if (pointer == 0 && button != 0) return false;
				if (disabled) return false;
				if (saulSelectBoxList.hasParent())
					hideList();
				else
					showList();
				return true;
	...
	public boolean isListDisplayed() {
		return saulSelectBoxList.hasParent();
	}
	...
	public void setMaxListCount (int maxListCount) {
		saulSelectBoxList.maxListCount = maxListCount;
	}

	/** @return Max number of items to display when the box is opened, or <= 0 to display them all. */
	public int getMaxListCount () {
		return saulSelectBoxList.maxListCount;
	}

	protected void setStage (Stage stage) {
		if (stage == null) saulSelectBoxList.hide();
	...
	public void setStyle (SelectBox.SelectBoxStyle style) {
		...
		this.style = style;
		if (saulSelectBoxList != null) {
			saulSelectBoxList.setStyle(style.scrollStyle);
			saulSelectBoxList.list.setStyle(style.listStyle);
		}
		invalidateHierarchy();
	}
	...
	public void setItems (T... newItems) {
		...
		selection.validate();
		saulSelectBoxList.list.setItems(items);
	...
	public void layout () {
		...
		if (saulSelectBoxList == null || !saulSelectBoxList.isScrollingDisabledY())
	...
	public void draw (Batch batch, float parentAlpha) {
		...
		else if (saulSelectBoxList.hasParent() && style.backgroundOpen != null)
	...
	/** @author Nathan Sweet */
	static class SaulSelectBoxList<T> extends ScrollPane {
		private final SaulSelectBox<T> selectBox;
		int maxListCount;
		...
		public SaulSelectBoxList(final SaulSelectBox<T> selectBox) {
		...
		public void show (Stage stage) {
			...
			if (getPrefHeight() > height && !this.isScrollingDisabledY()) width += getScrollBarWidth();
			setWidth(width);
