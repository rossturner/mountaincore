...
public class OldCraftingStationBehaviour {
	/*extends
} FurnitureBehaviour
		implements RequestHaulingAllocationMessage.ItemAllocationCallback,
	...
	public OldCraftingStationBehaviour() {

	}

	public OldCraftingStationBehaviour(CraftingType craftingType, JobType craftItemJobType, JobType haulingJobType,
	...
	@Override
	public OldCraftingStationBehaviour clone(MessageDispatcher messageDispatcher, GameContext gameContext) {
	...
	public void productionAssignmentCallback(List<CraftingRecipe> potentialCraftingRecipes) {
		for (CraftingRecipe potentialCraftingRecipe : potentialCraftingRecipes) {
			if (potentialCraftingRecipe.getInput().stream().anyMatch(QuantifiedItemTypeWithMaterial::isLiquid) ||
					potentialCraftingRecipe.getOutput().isLiquid()) {
	...
	public void jobCompleted(GameContext gameContext, Entity completedByEntity) {
		...
		if (currentProductionAssignment == null || currentProductionAssignment.targetRecipe == null) {
			...
			QuantifiedItemTypeWithMaterial outputRequirement = currentProductionAssignment.targetRecipe.getOutput();
		...
		if (liquidContainerComponent != null &&
				!currentProductionAssignment.targetRecipe.getOutput().isLiquid()) {
	...
	public List<I18nText> getDescription(I18nTranslator i18nTranslator, GameContext gameContext, MessageDispatcher messageDispatcher) {
		...
		if (currentProductionAssignment != null) {
			...
			QuantifiedItemTypeWithMaterial output = currentProductionAssignment.targetRecipe.getOutput();
			if (output.isLiquid()) {
	...
	public void readFrom(JSONObject asJson, SavedGameStateHolder savedGameStateHolder, SavedGameDependentDictionaries relatedStores) throws InvalidSaveException {
		...
		if (productionAssignmentId != null) {
			this.currentProductionAssignment = savedGameStateHolder.craftingAssignments.get(productionAssignmentId);
