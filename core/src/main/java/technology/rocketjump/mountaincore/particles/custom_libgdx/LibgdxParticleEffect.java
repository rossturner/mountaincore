 ...
 * @author mzechner */
public class LibgdxParticleEffect implements ParticleEffect, Disposable {
	...
	public LibgdxParticleEffect(boolean isAffectedByLighting) {
	...
	public void adjustForParentOrientation(EntityAssetOrientation effectDefaultOrientation, EntityAssetOrientation parentOrientation) {
		getEmitters().forEach(e -> {
			float angleHighMin = e.getAngle().getHighMin();
			float angleHighMax = e.getAngle().getHighMax();

			if (effectDefaultOrientation.equals(EntityAssetOrientation.LEFT)) {
				if (parentOrientation.equals(EntityAssetOrientation.RIGHT)) {
					// flip angle along Y axis
					e.getAngle().setHigh(
							180 - angleHighMax,
							180 - angleHighMin
					);
				} else if (parentOrientation.equals(EntityAssetOrientation.UP)) {
					float angleDiff = angleHighMax - angleHighMin;
					e.getAngle().setHigh(
							270 - (angleDiff / 2f),
							270 + (angleDiff / 2f)
					);
				} else if (parentOrientation.equals(EntityAssetOrientation.DOWN)) {
					float angleDiff = angleHighMax - angleHighMin;
					e.getAngle().setHigh(
							90 - (angleDiff / 2f),
							90 + (angleDiff / 2f)
