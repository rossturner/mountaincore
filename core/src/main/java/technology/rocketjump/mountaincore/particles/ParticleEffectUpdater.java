...
import static technology.rocketjump.mountaincore.jobs.model.JobTarget.NULL_TARGET;
import static technology.rocketjump.mountaincore.particles.CustomEffectFactory.DEFENSE_POOL_EFFECT_TYPE_NAME;
import static technology.rocketjump.mountaincore.particles.CustomEffectFactory.PROGRESS_BAR_EFFECT_TYPE_NAME;
...
//			Logger.warn("Not yet implemented - particles not attached to an entity");
	...
	private boolean insideBounds(Vector2 position) {
		if (cameraIsAtMaxZoom) {
			...
			return currentBoundingBox.contains(VectorUtils.toGridPoint(position));
		}
	}
