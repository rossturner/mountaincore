...
public class ParticleEmitter {
	...
	private Array<Sprite> diffuseSprites;
	private Array<Sprite> normalSprites;

	private Particle[] diffuseParticles;
	private Particle[] normalParticles;
	...
	private boolean usesNormalsRendering;

	public ParticleEmitter (boolean usesNormalsRendering) {
		this.usesNormalsRendering = usesNormalsRendering;
		initialize();
	}

	public ParticleEmitter(BufferedReader reader, boolean usesNormalsRendering) throws IOException {
		this.usesNormalsRendering = usesNormalsRendering;
		initialize();
		load(reader);
	}
	...
	public ParticleEmitter (ParticleEmitter emitter) {
		diffuseSprites = new Array<>(emitter.diffuseSprites);
		usesNormalsRendering = emitter.usesNormalsRendering;
		if (usesNormalsRendering) {
			normalSprites = new Array<>(emitter.normalSprites);
		}
	...
	private void initialize () {
		diffuseSprites = new Array<>();
		if (usesNormalsRendering) {
			normalSprites = new Array<>();
		}
	...
	public void setMaxParticleCount (int maxParticleCount) {
		...
		activeCount = 0;
		diffuseParticles = new Particle[maxParticleCount];
		if (usesNormalsRendering) {
			normalParticles = new Particle[maxParticleCount];
		}
	}
	...
	public void update (float delta) {
		...
		for (int i = 0, n = active.length; i < n; i++) {
			if (active[i]) {
				boolean isAlive = updateParticle(this.diffuseParticles[i], diffuseSprites, delta, deltaMillis, RenderMode.DIFFUSE);
				if (usesNormalsRendering) {
					updateParticle(this.normalParticles[i], normalSprites, delta, deltaMillis, RenderMode.NORMALS);
				}
				if (!isAlive) {
					active[i] = false;
					activeCount--;
	...
	public void draw(Batch batch, RenderMode renderMode) {
		...
		Particle[] particles = renderMode.equals(RenderMode.DIFFUSE) ? this.diffuseParticles : this.normalParticles;
		boolean[] active = this.active;
	...
	private void restart () {
		...
		delay = delayValue.active ? delayValue.newLowValue(random) : 0;
		delayTimer = 0;

		durationTimer -= duration;
		duration = durationValue.newLowValue(random);

		emission = (int)emissionValue.newLowValue(random);
		emissionDiff = (int)emissionValue.newHighValue(random);
		if (!emissionValue.isRelative()) emissionDiff -= emission;

		if (!lifeValue.independent) generateLifeValues(random);

		if (!lifeOffsetValue.independent) generateLifeOffsetValues(random);

		spawnWidth = spawnWidthValue.newLowValue(random);
		spawnWidthDiff = spawnWidthValue.newHighValue(random);
		if (!spawnWidthValue.isRelative()) spawnWidthDiff -= spawnWidth;

		spawnHeight = spawnHeightValue.newLowValue(random);
		spawnHeightDiff = spawnHeightValue.newHighValue(random);
	...
	protected Particle[] getDiffuseParticles () {
		return diffuseParticles;
	}

	protected Particle[] getNormalParticles() {
		return normalParticles;
	}

	private void activateParticle (int index) {
		long seed = new RandomXS128().nextLong();
		activateParticle(index, diffuseSprites, diffuseParticles, new RandomXS128(seed), RenderMode.DIFFUSE);
		if (usesNormalsRendering) {
			activateParticle(index, normalSprites, normalParticles, new RandomXS128(seed), RenderMode.NORMALS);
		}
	}

	private void activateParticle(int index, Array<Sprite> sprites, Particle[] particles, RandomXS128 random, RenderMode renderMode) {
		Sprite sprite = null;
		switch (spriteMode) {
			case single:
			case animated:
		...
		if (particle == null) {
			...
			if (RenderMode.NORMALS.equals(renderMode)) {
		...
		if (velocityValue.active) {
			particle.velocity = velocityValue.newLowValue(random);
			particle.velocityDiff = velocityValue.newHighValue(random);
			if (!velocityValue.isRelative()) particle.velocityDiff -= particle.velocity;
		}

		particle.angle = angleValue.newLowValue(random);
		particle.angleDiff = angleValue.newHighValue(random);
		...
		particle.xScale = xScaleValue.newLowValue(random) / spriteWidth;
		particle.xScaleDiff = xScaleValue.newHighValue(random) / spriteWidth;
		if (!xScaleValue.isRelative()) particle.xScaleDiff -= particle.xScale;

		if (yScaleValue.active) {
			particle.yScale = yScaleValue.newLowValue(random) / spriteHeight;
			particle.yScaleDiff = yScaleValue.newHighValue(random) / spriteHeight;
		...
		if (rotationValue.active) {
			particle.rotation = rotationValue.newLowValue(random);
			particle.rotationDiff = rotationValue.newHighValue(random);
		...
		if (windValue.active) {
			particle.wind = windValue.newLowValue(random);
			particle.windDiff = windValue.newHighValue(random);
			if (!windValue.isRelative()) particle.windDiff -= particle.wind;
		}

		if (gravityValue.active) {
			particle.gravity = gravityValue.newLowValue(random);
			particle.gravityDiff = gravityValue.newHighValue(random);
		...
		float[] temp = renderMode.equals(RenderMode.DIFFUSE) ? tintValue.getColor(0) : WHITE;
		color[0] = temp[0];
		color[1] = temp[1];
		color[2] = temp[2];

		particle.transparency = transparencyValue.newLowValue(random);
		particle.transparencyDiff = transparencyValue.newHighValue(random) - particle.transparency;

		// Spawn.
		float x = this.x;
		if (xOffsetValue.active) x += xOffsetValue.newLowValue(random);
		float y = this.y;
		if (yOffsetValue.active) y += yOffsetValue.newLowValue(random);
		switch (spawnShapeValue.shape) {
			...
			case ellipse: {
				...
				if (spawnShapeValue.edges) {
					...
					switch (spawnShapeValue.side) {
						case top:
							spawnAngle = -random.nextFloat() * 179f;
							break;
						case bottom:
							spawnAngle = random.nextFloat() * 179f;
							break;
						default:
							spawnAngle = random.nextFloat() * 360f;
							break;
					}
					...
					while (true) {
						float px = (random.nextFloat() * width) - radiusX;
						float py = (random.nextFloat() * width) - radiusX;
			...
			case line: {
				...
				float height = spawnHeight + (spawnHeightDiff * spawnHeightValue.getScale(percent));
				if (width != 0) {
					float lineX = width * random.nextFloat();
					x += lineX;
		...
		if (offsetTime > 0) {
			...
			updateParticle(particle, sprites, offsetTime / 1000f, offsetTime, renderMode);
		}
	}

	private boolean updateParticle(Particle particle, Array<Sprite> sprites, float delta, int deltaMillis, RenderMode renderMode) {
		...
		float[] color;
		if ((updateFlags & UPDATE_TINT) != 0 && renderMode.equals(RenderMode.DIFFUSE))
	...
	private void generateLifeValues (Random random) {
		life = (int)lifeValue.newLowValue(random);
		lifeDiff = (int)lifeValue.newHighValue(random);
		if (!lifeValue.isRelative()) lifeDiff -= life;
	}

	private void generateLifeOffsetValues(Random random) {
		lifeOffset = lifeOffsetValue.active ? (int)lifeOffsetValue.newLowValue(random) : 0;
		lifeOffsetDiff = (int)lifeOffsetValue.newHighValue(random);
	...
	public void setPosition (float x, float y) {
		if (attached) {
			...
			for (int i = 0, n = active.length; i < n; i++)
				if (active[i]) {
					diffuseParticles[i].translate(xAmount, yAmount);
					if (usesNormalsRendering) {
						normalParticles[i].translate(xAmount, yAmount);
					}
				}
		}
		this.x = x;
	...
	public void setSprites(Array<Sprite> diffuseSprites, Array<Sprite> normalSprites) {
		...
		for (int i = 0, n = diffuseParticles.length; i < n; i++) {
			...
			if (diffuseParticle == null) break;
			Particle normalParticle = usesNormalsRendering ? normalParticles[i] : null;
			Sprite diffuseSprite = null;
			Sprite normalSprite = null;
			switch (spriteMode) {
			case single:
				diffuseSprite = diffuseSprites.first();
				if (usesNormalsRendering) {
					normalSprite = normalSprites.first();
				}
				break;
			case random:
				Random random = new RandomXS128();
				int index = random.nextInt(diffuseSprites.size);
				diffuseSprite = diffuseSprites.get(index);
				if (usesNormalsRendering) {
					normalSprite = normalSprites.get(index);
				}
				break;
			case animated:
				float percent = 1 - diffuseParticle.currentLife / (float)diffuseParticle.life;
				diffuseParticle.frame = Math.min((int)(percent * diffuseSprites.size), diffuseSprites.size - 1);
				diffuseSprite = diffuseSprites.get(diffuseParticle.frame);
				if (usesNormalsRendering) {
					normalParticle.frame = Math.min((int)(percent * normalSprites.size), normalSprites.size - 1);
					normalSprite = normalSprites.get(normalParticle.frame);
				}
				break;
			}
			diffuseParticle.setRegion(diffuseSprite);
			diffuseParticle.setOrigin(diffuseSprite.getOriginX(), diffuseSprite.getOriginY());
			if (usesNormalsRendering) {
				normalParticle.setRegion(normalSprite);
				normalParticle.setOrigin(normalSprite.getOriginX(), normalSprite.getOriginY());
			}
		}
	}
	...
	public Array<Sprite> getDiffuseSprites () {
		return diffuseSprites;
	}

	public Array<Sprite> getNormalSprites() {
	...
	public void setFlip (boolean flipX, boolean flipY) {
		this.flipX = flipX;
		this.flipY = flipY;
		if (diffuseParticles == null) return;
		for (int i = 0, n = diffuseParticles.length; i < n; i++) {
			Particle particle = diffuseParticles[i];
			if (particle != null) particle.flip(flipX, flipY);
			if (usesNormalsRendering) {
				Particle normalParticle = normalParticles[i];
				if (normalParticle != null) normalParticle.flip(flipX, flipY);
			}
		}
	}
	...
	public BoundingBox getBoundingBox () {
		...
		Particle[] particles = this.diffuseParticles;
		boolean[] active = this.active;
	...
	static public class RangedNumericValue extends ParticleValue {
		...
		public float newLowValue(Random random) {
			return lowMin + (lowMax - lowMin) * random.nextFloat();
		}
	...
	static public class ScaledNumericValue extends RangedNumericValue {
		...
		public float newHighValue(Random random) {
			return highMin + (highMax - highMin) * random.nextFloat();
		}
