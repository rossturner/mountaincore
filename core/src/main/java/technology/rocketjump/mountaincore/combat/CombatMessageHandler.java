...
import technology.rocketjump.mountaincore.assets.entities.tags.ExtraFurnitureHitpointsTag;
import technology.rocketjump.mountaincore.assets.entities.tags.FlatDamageReductionTag;
import technology.rocketjump.mountaincore.combat.model.WeaponAttack;
import technology.rocketjump.mountaincore.entities.FireMessageHandler;
import technology.rocketjump.mountaincore.entities.ai.combat.*;
...
import technology.rocketjump.mountaincore.entities.behaviour.items.ProjectileBehaviour;
...
import technology.rocketjump.mountaincore.entities.model.physical.creature.body.*;
import technology.rocketjump.mountaincore.entities.model.physical.creature.body.organs.OrganDamageEffect;
import technology.rocketjump.mountaincore.entities.model.physical.creature.body.organs.OrganDamageLevel;
import technology.rocketjump.mountaincore.entities.model.physical.creature.status.*;
...
import static technology.rocketjump.mountaincore.entities.model.physical.creature.body.BodyPartDamageLevel.Destroyed;
...
public class CombatMessageHandler implements Telegraph, GameContextAware {
	...
	private void triggerAttackOfOpportunity(Entity fleeingEntity) {
		for (Entity opponentEntity : CombatBehaviour.getOpponentsInMelee(fleeingEntity, gameContext)) {
	...
	private void createProjectile(CombatAttackMessage attackMessage) {
		...
		Entity projectileEntity = itemEntityFactory.create(ammoAttributes, VectorUtils.toGridPoint(attackerLocation), false, gameContext, Faction.SETTLEMENT);
		...
		if (attackMessage.defenderEntity.getPhysicalEntityComponent().getAttributes() instanceof CreatureEntityAttributes defenderAttributes) {
			...
			if (attributes.getDamageAmount() > damageToDestroyFurniture(attackMessage.defenderEntity)) {
				messageDispatcher.dispatchMessage(MessageType.DAMAGE_FURNITURE, new FurnitureDamagedMessage(
						...
						FireMessageHandler.blackenedColor(gameContext.getRandom()), FireMessageHandler.blackenedColor(gameContext.getRandom())
				));
			}
		} else {
	...
	private void handleKnockback(CombatAttackMessage attackMessage) {
		if (attackMessage.defenderEntity.getBehaviourComponent() instanceof CreatureBehaviour defenderBehaviour && shouldApplyKnockback(defenderBehaviour)) {
			GridPoint2 defenderTilePosition = VectorUtils.toGridPoint(attackMessage.defenderEntity.getLocationComponent().getWorldOrParentPosition());
							...
							tile.getEntities().stream()
									.noneMatch(entity -> {
										...
										if (combatStateComponent == null || !combatStateComponent.isInCombat() || combatStateComponent.getHeldLocation() == null) {
											...
											return combatStateComponent.getHeldLocation().equals(VectorUtils.toGridPoint(entity.getLocationComponent().getWorldOrParentPosition()));
										}
									})
					).toList();
	...
	public static List<EntityAssetOrientation> getOrientationsOppositeTo(EntityAssetOrientation facingOrientation) {
		return switch (facingOrientation) {
			case UP -> List.of(EntityAssetOrientation.DOWN_LEFT, EntityAssetOrientation.DOWN, EntityAssetOrientation.DOWN_RIGHT);
			case UP_RIGHT -> List.of(EntityAssetOrientation.LEFT, EntityAssetOrientation.DOWN_LEFT, EntityAssetOrientation.DOWN);
			case RIGHT -> List.of(EntityAssetOrientation.UP_LEFT, EntityAssetOrientation.LEFT, EntityAssetOrientation.DOWN_LEFT);
			case DOWN_RIGHT -> List.of(EntityAssetOrientation.LEFT, EntityAssetOrientation.UP_LEFT, EntityAssetOrientation.UP);
			case DOWN -> List.of(EntityAssetOrientation.UP_LEFT, EntityAssetOrientation.UP, EntityAssetOrientation.UP_RIGHT);
			case DOWN_LEFT -> List.of(EntityAssetOrientation.UP, EntityAssetOrientation.UP_RIGHT, EntityAssetOrientation.RIGHT);
			case LEFT -> List.of(EntityAssetOrientation.UP_RIGHT, EntityAssetOrientation.RIGHT, EntityAssetOrientation.DOWN_RIGHT);
			case UP_LEFT -> List.of(EntityAssetOrientation.RIGHT, EntityAssetOrientation.DOWN_RIGHT, EntityAssetOrientation.DOWN);
		};
	}
