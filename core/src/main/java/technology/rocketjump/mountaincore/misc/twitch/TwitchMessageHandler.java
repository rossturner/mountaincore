	...
	public boolean handleMessage(Telegram msg) {
		switch (msg.message) {
			...
			case MessageType.TWITCH_TOKEN_UPDATED: {
				...
				try {
					if (token == null) {
						userPreferences.removePreference(UserPreferences.PreferenceKey.TWITCH_TOKEN);
					} else {
						userPreferences.setPreference(UserPreferences.PreferenceKey.TWITCH_TOKEN, new ObjectMapper().writeValueAsString(token));
					}
