...
import technology.rocketjump.mountaincore.mapping.OutdoorLightProcessor;
		...
		for (int x = 0; x < generatedSourceMap.getWidth(); x++) {
			for (int y = 0; y < generatedSourceMap.getHeight(); y++) {
				...
				if (sourceTile.getTileType().equals(TileType.MOUNTAIN)) {
					...
					} else if (sourceTile.getFloorType().equals(technology.rocketjump.mountaincore.mapgen.model.FloorType.Rock)) {
						if (sourceTile.getTileSubType().equals(TileSubType.LOAMY_FLOOR_CAVE)) {
					...
					switch (sourceTile.getSubRegion().getSubRegionType()) {
						...
						case PLAINS -> {
							if (orthogonallyBorders(TileSubType.TUNDRA, sourceTile, generatedSourceMap) && random.nextFloat() < 0.7f) {
								targetTile.getFloor().setFloorType(dirtFloorType);
							} else {
								targetTile.getFloor().setFloorType(grassFloorType);
							}
