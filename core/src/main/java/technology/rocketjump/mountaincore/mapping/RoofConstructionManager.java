...
import technology.rocketjump.mountaincore.messaging.types.RoofCollapseMessage;
...
public class RoofConstructionManager implements GameContextAware {
	...
	public void roofConstructionAdded(MapTile mapTile) {
		...
		if (!withinRangeOfSupport(mapTile, false)) {
			switchState(mapTile, RoofConstructionState.TOO_FAR_FROM_SUPPORT);
		} else {
	...
	public void roofConstructionRemoved(MapTile mapTile) {
		switchState(mapTile, RoofConstructionState.NONE);
	}

	public void roofDeconstructionAdded(MapTile mapTile) {
		switchState(mapTile, RoofConstructionState.PENDING_DECONSTRUCTION);
	}
	...
	public void roofConstructed(MapTile mapTile) {
		for (CompassDirection direction : CompassDirection.CARDINAL_DIRECTIONS) {
			...
			if (tile != null && tile.getRoof().getConstructionState().equals(RoofConstructionState.NO_ADJACENT_ROOF)) {
				switchState(tile, RoofConstructionState.READY_FOR_CONSTRUCTION);
			}
		}
	}
	...
	public void supportConstructed(MapTile mapTile) {
		for (int yCursor = mapTile.getTileY()-ROOF_SUPPORT_MAX_DISTANCE; yCursor <= mapTile.getTileY()+ROOF_SUPPORT_MAX_DISTANCE; yCursor++) {
			for (int xCursor = mapTile.getTileX()-ROOF_SUPPORT_MAX_DISTANCE; xCursor <= mapTile.getTileX()+ROOF_SUPPORT_MAX_DISTANCE; xCursor++) {
				...
				if (tile != null && tile.getRoof().getState().equals(TileRoofState.OPEN) && tile.getRoof().getConstructionState().equals(RoofConstructionState.TOO_FAR_FROM_SUPPORT)) {
					switchState(tile, RoofConstructionState.NO_ADJACENT_ROOF);
	...
	private void switchState(MapTile mapTile, RoofConstructionState newState) {
		...
		if (oldState.equals(RoofConstructionState.READY_FOR_CONSTRUCTION) || oldState.equals(RoofConstructionState.PENDING_DECONSTRUCTION)) {
		...
		} else if (newState.equals(RoofConstructionState.PENDING_DECONSTRUCTION)) {
