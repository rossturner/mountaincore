...
public class SimplexNoiseOctave {  // Simplex noise in 2D, 3D and 4D
	...
	public SimplexNoiseOctave(Random random) {
		System.arraycopy(p_supply, 0, p, 0, p_supply.length);
//		p=p_supply.clone();

		//the seed determines the swaps that occur between the default order and the order we're actually going to use
		for(int i=0;i<NUMBEROFSWAPS;i++){
			int swapFrom=random.nextInt(p.length);
			int swapTo=random.nextInt(p.length);
