...
import technology.rocketjump.mountaincore.mapgen.model.input.MushroomType;
import technology.rocketjump.mountaincore.mapgen.model.output.GameMap;
import technology.rocketjump.mountaincore.mapgen.model.output.GameMapTile;
import technology.rocketjump.mountaincore.mapgen.model.output.MapSubRegion;
import technology.rocketjump.mountaincore.mapgen.model.output.TileSubType;
...
public class MushroomSpawner {
	...
	public void placeMushrooms(GameMap map, Random random, GameMapGenerationParams generationParams) {
		...
		for (MapSubRegion subRegion : map.getSubRegions().values()) {
			if (subRegion.getSubRegionType().equals(TileSubType.STONE_FLOOR_CAVE)) {
		...
		while (numMushroomsToPlaceRandomly > 0 && allCaveTiles.size() > 0) {
			...
			for (GameMapTile neighbourTile : map.getAllNeighbourTiles(mushroomTile.getPosition().x, mushroomTile.getPosition().y)) {
				if (neighbourTile.getTileSubType().equals(TileSubType.STONE_FLOOR_CAVE)) {
					neighbourTile.setTileSubType(TileSubType.LOAMY_FLOOR_CAVE);
		...
		while (numMushroomsToPlaceNearOthers > 0 && !mushroomLocations.isEmpty()) {
			...
			while (attempts < 5) {
				...
				if (nearbyTile != null && nearbyTile.getTileSubType().equals(TileSubType.LOAMY_FLOOR_CAVE) && !nearbyTile.hasMushroom()) {
	...
	private void addMoreLoam(List<GameMapTile> loamyTiles, List<GameMapTile> allCaveTiles, GameMap map, Random random) {
		...
		while (neighbourIterator.hasNext() && (loamyTiles.size() + moreLoamyTiles.size()) < NUM_LOAMY_FLOOR_SURROUNDING_MUSHROOMS) {
			...
			for (GameMapTile furtherAwayTile : moreTiles) {
				if (furtherAwayTile.getTileSubType().equals(TileSubType.STONE_FLOOR_CAVE)) {
					furtherAwayTile.setTileSubType(TileSubType.LOAMY_FLOOR_CAVE);
