...
import static technology.rocketjump.mountaincore.entities.model.physical.furniture.FurnitureLayout.SpecialTile.SpecialTileRequirment.IS_RIVER;
		...
		if (parentEntity.getType().equals(EntityType.FURNITURE)) {
			...
			if (!currentLayout.getSpecialTiles().isEmpty()) {
				...
				if (specialTile.getRequirement().equals(IS_RIVER)) {
					GridPoint2 specialTileLocation = VectorUtils.toGridPoint(parentEntity.getLocationComponent().getWorldPosition()).add(specialTile.getLocation());
