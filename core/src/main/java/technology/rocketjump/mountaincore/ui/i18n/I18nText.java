...
import static technology.rocketjump.mountaincore.ui.widgets.tooltips.I18nTextElement.singleSpace;
...
public class I18nText implements I18nString {
	...
	public I18nText tidy(boolean firstInvocation) {
		if (firstInvocation && textElements.size() > 1 && textElements.stream().allMatch(e -> e.getTooltipI18nKey() == null)) {
			...
			String combinedText = textElements.stream().map(I18nTextElement::getText).collect(Collectors.joining()).replaceAll(" +", " ").replaceFirst("^ +", "");
			textElements.clear();
