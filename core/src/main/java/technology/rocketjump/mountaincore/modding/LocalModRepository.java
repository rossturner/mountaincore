...
import technology.rocketjump.mountaincore.modding.model.ModioMetadata;
...
public class LocalModRepository {
	...
	public void updateLocalModListing() {
		...
		for (String activeModName : activeModsString.split(MOD_NAME_SEPARATOR)) {
			...
			if (activeMod != null) {
				if (modCompatibilityChecker.checkCompatibility(activeMod).compatibility().equals(ModCompatibilityChecker.Compatibility.INCOMPATIBLE)) {
	...
	private void createModioMeta(Path unzippedMod, WrappedModioJson mod) throws IOException {
		Path modioPath = unzippedMod.resolve(ModParser.MODIO_JSON_FILENAME);
