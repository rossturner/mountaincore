package technology.rocketjump.mountaincore.doors;
...
import technology.rocketjump.mountaincore.entities.behaviour.furniture.DoorBehaviour;
...
import static technology.rocketjump.mountaincore.doors.DoorwayOrientation.EAST_WEST;
import static technology.rocketjump.mountaincore.doors.DoorwayOrientation.NORTH_SOUTH;
import static technology.rocketjump.mountaincore.mapping.MapMessageHandler.updateTile;
...
public class DoorwayMessageHandler implements GameContextAware, Telegraph {
	...
	private Entity buildWallCap(GridPoint2 wallTileLocation, Doorway doorway, EntityAssetOrientation assetOrientation, float positionYOffset) {
		...
		wallCapEntity.getLocationComponent().setWorldPosition(VectorUtils.toVector(doorway.getTileLocation()).add(0, positionYOffset), false, false);
	...
	private Entity createFrameEntity(DoorwayPlacementMessage message, EntityAssetOrientation orientation, float positionYOffset) {
		...
		entity.getLocationComponent().setWorldPosition(VectorUtils.toVector(message.getTilePosition()).add(0, positionYOffset), false, false);
		return entity;
	}
	...
	private Entity createDoorEntity(DoorwayPlacementMessage message) {
		...
		entity.getLocationComponent().setWorldPosition(VectorUtils.toVector(message.getTilePosition()).add(0, 0.5f - (1f/PIXELS_PER_TILE)), false, false);
		return entity;
	}
